...
Load the code base, or access it via your Terminal/CMD/Powershell. Select the correct folder for the service you wish to run. First run a 'gradle clean', a 'gradle build', and then a 'gradle run' command. This will execute the service.
...
As with gradle, navigate to the correct directory for the service you wish to use.

1. Build the Docker image using the provided Dockerfile. This can be done with the command: `docker build -t your-image-name .`
2. You must have docker running on the grading machine
3. Run the service using docker run -it -p 8080:8080 my-service:latest
4. The program then terminates, but can be run indefinetly using step 1. It automatically overwrites previous checks and files every run.
